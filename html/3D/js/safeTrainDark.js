...
var barXdata = ['南宁市','柳州市','桂林市','梧州市','北海市','防港城市','钦州市','贵港市','玉林市','百色市','贺州市','河池市','来宾市','崇左市'],
    ...
    titles = ["全区范围最新安全培训教育课程TOP20","全市范围最新安全培训教育课程TOP20","全县范围最新安全培训教育课程TOP20"],
...
function loadSwiper() {
    ...
    for(var i = 0; i < l; i++){
        data.push({
            name1: '企业名称企业名称' + (Math.random()*100).toFixed(0),
            name2: '课程名称课程名称课程名称课程名称课程名称课程名称课程名称课程名称课程名称课程名称',
            name3: '2019-08-25 12:00:00',
            name4: '1'
        });
    }
    var _html = '';
    data.forEach(function (item,index) {
        _html += '<div class="swiper-slide">\n' +
            ...
            '    <p class="long-p">课程名称：<span>' + item.name2 + '</span></p>\n' +
            '    <p>培训时间：<span>' + item.name3 + '</span></p>\n' +
            '    <p>培训人数：<span>' + item.name4 + '</span></p>\n' +
            '</div>\n' +
            '</div>\n' +
            '                </div>';
    });
...
function setOverflow() {
    var h = $(".each-meeting").height(),lineHeight = parseFloat($(".each-meeting p").css("line-height"));
    $(".long-p").each(function () {
        var restH = h*0.94;
        $(this).siblings("p").each(function () {
            var mt = $(this).css("margin-top");
            restH = restH - $(this).height() + parseFloat(mt);
        });
        // $(this).css("-webkit-line-clamp",Math.floor((restH - 10)/lineHeight));   //直接设置不成功
        if((restH - lineHeight) < 0 ){
            $(this).addClass("clamp0");
        }else{
            $(this).addClass("clamp" + Math.floor((restH - lineHeight)/lineHeight));
        }
    });
}