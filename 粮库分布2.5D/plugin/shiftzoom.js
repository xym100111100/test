var cvi_sztimer, cvi_szactive,videoList = [],lists= [],video = true,winWidth = $(window).width(),winHeight = $(window).height(),times = 0.254,times1 = 1,oldX, cvi_szimage = null, shiftzoom = {
		...
		if (!ele.active) {
			...
			if (document.images && document.createElement
				...
				var st, over, view, div = ele.parentNode, img = shiftzoom
						...
						gView = shiftzoom.E('div'),aView = shiftzoom.E('a'),outer = shiftzoom.E('div');
				...
				if (ele.width >= 100 && ele.width < img.maxwidth
						...
						for(var i in videoList) {
							...
							if (row.type) {
								if(row.type.value == "90033-10"){
									videoView.className = 'camera-box scamera-box';
								}else if(row.type.value == "90033-20"){
									videoView.className = 'camera-box ocamera-box';
								}
							} else {
								videoView.className = 'camera-box scamera-box';
							}
							...
							aView.innerHTML = '<i></i>' + row.sourceName;
							videoView.appendChild(aView);
							granary.appendChild(videoView);
						}
					}
					/*var wRatio = Math.round(width/imgWidth*10000)/10000;
					var hRatio = Math.round(height/imgHeight*10000)/10000;
					var ratio = wRatio> hRatio ? wRatio : hRatio;
					var floodList = [{xAxis:'1080',yAxis:'626',id:1},
						{xAxis:'2060',yAxis:'490',id:2},
						{xAxis:'2378',yAxis:'1246',id:3},
						{xAxis:'2980',yAxis:'945',id:4}]
					for (var i = 0; i < floodList.length; i ++) {
						var row = floodList[i];
						videoView = shiftzoom.E('div');
						videoView.id = "flood"+row.id;
						videoView.className = 'i-sw';
						st = videoView.style;
						st.left = (xs+1)*Math.round((row.xAxis)*ratio) + 'px';
						st.top = (ys+1)* Math.round((row.yAxis)*ratio) + 'px';
						st.zIndex = 3;
						videoView.setAttribute("onclick",
								"gotoFlood("+row.id+")");
						granary.appendChild(videoView);
					}*/
                    ...
                    if (vLists.length>0) {
                        $.each(vLists,function(val) {
                            var row = vLists[val], id = "";
                            var vw = shiftzoom.E('div');
                            if (row.id) {
                                id = row.id;
                            } else {
                                id = row.name + "".replace('号摄像头', '') + "empty";
                            }
                            vw.id = 'video' + id;
                            vw.className = 'video-state';
                            st = vw.style;
                            st.left = Math.round((parseInt(row.xAxis)) * ratio) + 'px';
                            st.top = Math.round((parseInt(row.yAxis)) * ratio) + 3 + 'px';
                            st.zIndex = 2;
                            granary.appendChild(vw);
                            aView = shiftzoom.E('a');
                            aView.id = 'video_'+id;
                            if (parseInt(row.canClick)) {
                                aView.href = 'javascript:showVideo('+id+');';
                            }
                            vw.appendChild(aView);
                            var video = shiftzoom.E('img');
                            video.src = 'images/video.png';
                            aView.appendChild(video);
                        });
					}
					...
					if (lists.length>0) {
						$.each(lists,function(val){
							...
							if (row.id) {
								...
								id = row.name+"".replace('号仓','').replace('库','')+"empty";
							}
							granaryView.id = 'warehouse'+id;
							granaryView.className = 'silo-state';
							if (row.showGasPh3Wran) {
								granaryView.className += " silo-alarm";
							}
							st = granaryView.style;
							st.left = (xs+1)*row.x + 'px';
							st.top = (ys+1)*row.y + 'px';
							st.zIndex = 2;
							...
							granaryView.appendChild(aView);

							alarmView = shiftzoom.E('div');
							alarmView.className = 'dot-spread';
							alarmView.id = 'spread_'+id;
							if (row.showGasPh3Wran) {
								alarmView.style.display = 'block';
							} else {
								alarmView.style.display = 'none';
							}
							aView.appendChild(alarmView);
							armView = shiftzoom.E('div');
							armView.className = 'dot';
							alarmView.appendChild(armView);
							armView = shiftzoom.E('div');
							armView.className = 'pulse';
							alarmView.appendChild(armView);
							armView = shiftzoom.E('div');
							armView.className = 'pulse1';
							alarmView.appendChild(armView);
							...
							iView = shiftzoom.E('div');
							iView.className = 'temp';
							iView.id = 'showTemp'+row.id;
							if (row.showTempWran) {
								iView.className += ' c-red';
							}
							iView.title = '库存';
                            iView.innerHTML = row.stock;
                            view.appendChild(iView);

							if (row.name.indexOf('油罐') == -1) {
								sView = shiftzoom.E('s');
								sView.className = 'icon-state icon-state-abno';
					...
					st.position = 'absolute';
					var bottom = 0;
					var isIE = shiftzoom._isIE();
					if (img.minheight != winHeight) {
						bottom = Math.round(img.minheight-(winHeight+17));
						if (!isIE) {
							bottom = bottom/2;
						} else {
							bottom = bottom + 17;
						...
						bottom = Math.round(img.minheight-(winHeight))/2;
						if (!isIE) {
							bottom = bottom/2;
						}
					}
					st.bottom =  bottom +"px";
					st.right = parseInt(img.minwidth-(winWidth))+"px";
					over.unselectable = "on";
					div.appendChild(over);
					...
					st.bottom = '0px';
					st.right = '0px';
					st.border = '1px solid #7eaec1';
					st.msInterpolationMode = img.bicubic;
					over.appendChild(view);
					img.tumbid = view.id;
					view.onmousedown = shiftzoom._catchDrag;
					view = shiftzoom.E('span');
					view.id = img.id + '_view';
					view.maxleft = 0;
					view.maxtop = 0;
					st = view.style;
					st.position = "absolute";
					st.height = parseInt((outer.height * img.ovsfact)
							- (img.bmode ? 0 : 2))
							+ 'px';
					st.width = parseInt((outer.width * img.ovsfact)
							- (img.bmode ? 0 : 2))
							+ 'px';
					st.width = '60px';
					st.height = '29px';
					st.padding = '3px';
					var src = xref.src;
					st.background = "url("+ src.substring(0,src.lastIndexOf("/"))+"/img-area.png) no-repeat";
					over.appendChild(view);*/
	...
	_setOverview : function(img) {
		var wRatio = Math.round(img.minwidth/img.clientWidth*10000)/10000;
 		var hRatio = Math.round(img.minheight/img.clientHeight*10000)/10000;
 		var ratio = wRatio> hRatio ? wRatio : hRatio;
		var view = shiftzoom.G(img.viewid);
		view.style.width = 230*ratio + 'px';
		view.style.height = 137 * ratio + 'px';
		...
		view.style.top = Math
				.round((Math.abs(img.parentNode.top) / (img.parentNode.height / img.minheight))
						* img.ovsfact)
				- (img.bmode ? 2 : 0) + 'px';
		view.maxleft = (230-(230*ratio));
		view.maxtop = (137-(137*ratio));
		return false;
	},
	...
	zoomChange : function(img,mw,mh) {
			...
			for(var i in videoList) {
				...
				if (warehouse) {
					...
					if(row.type && row.type.value == "90033-10"){
						warehouse.className = 'camera-box scamera-box';
					}else if(row.type && row.type.value == "90033-20"){
		...
		$.each(lists,function(val){
			...
			warehouse = shiftzoom.G('warehouse'+id);
			...
			if (warehouse) {
				...
				gasView = shiftzoom.G('spread_'+id);
				if (row.showGasPh3Wran) {
					warehouse.className += " silo-alarm";
					gasView.style.display = 'block';
				}
				gasView.style.display = 'none';
				tempView = shiftzoom.G('showTemp'+id);
				if (tempView != null) {
					tempView.className = 'temp';
					if (row.showTempWran) {
						tempView.className += ' c-red';
					}
				}
				
				/*style.left = Math.round(mx * row.x *100)/100+'px';
				style.top = Math.round(my * (row.y+divHeight) * 100)/100 - divHeight +'px';*/
				style.left = Math.round((parseInt(row.xAxis))*ratio)-(divHeight/2)+'px';
				style.top = Math.round((parseInt(row.yAxis))*ratio)-divHeight+3+'px';
			...
			if(vLists.length > 0){
                $.each(vLists,function(val) {
                    ...
                    video = shiftzoom.G('video' + id);

                    if (video) {
                        var style = video.style;
                        video.className = 'video-state';
                        style.left = Math.round((parseInt(row.xAxis)) * ratio) + 'px';
                        style.top = Math.round((parseInt(row.yAxis)) * ratio) + 3 + 'px';
	...
	_stopPan : function() {
		...
		if (img.overview) {
			...
			var left = Math
					.round((Math.abs(parseInt(img.parentNode.left)) / (img.parentNode.width / img.minwidth))
							* img.ovsfact) + 8;
			var top = Math
					.round((Math.abs(parseInt(img.parentNode.top)) / (img.parentNode.height / img.minheight))
							* img.ovsfact) + 8;
							
			if (left >= shiftzoom.G(img.viewid).maxleft) {
				left = shiftzoom.G(img.viewid).maxleft;
			}
			...
			if (!parseInt(img.parentNode.style.top)) {
				top = 0;
			} else if (top >= shiftzoom.G(img.viewid).maxtop) {
				top = shiftzoom.G(img.viewid).maxtop;
			}
			view.top = top + 'px';
